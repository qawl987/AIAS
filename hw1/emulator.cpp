	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			//instruction added
      		case MUL:
			{
				int64_t result = static_cast<int64_t>(rf[i.a2.reg]) * static_cast<int64_t>(rf[i.a3.reg]);
				int32_t lower = static_cast<int32_t>(result & 0xFFFFFFFF);
				rf[i.a1.reg] = static_cast<uint32_t>(lower);
				break;
			}
			case MULHU:
			{
