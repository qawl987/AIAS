...
// class OneClockStack(val depth: Int) extends Module {
//   val io = IO(new Bundle {
//     val push    = Input(Bool())
//     val pop     = Input(Bool())
//     val en      = Input(Bool())
...
//   // Internal storage
//   val stack_mem = Reg(Vec(depth, UInt(32.W)))
//   val count_reg = RegInit(0.U(log2Ceil(depth + 1).W))
...
//   // Output logic - update dataOut_reg on clock edge
//   when (io.en && io.pop && (count_reg > 0.U)) {
//     // If popping, show the next item in stack (or 0 if empty)
//     dataOut_reg := Mux(count_reg > 1.U, stack_mem(next_top_ptr), 0.U)
//   } .elsewhen (count_reg > 0.U) {
//     // If not popping, show the current top
//     dataOut_reg := stack_mem(curr_top_ptr)
//   } .otherwise {
